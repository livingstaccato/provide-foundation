...
class TestAsyncCircuitBreakerReset(FoundationTestCase):
    ...
    @pytest.mark.asyncio
    async def test_async_breaker_reset_via_direct_instances(self) -> None:
        """Test that AsyncCircuitBreaker can be reset via _reset_direct_circuit_breaker_instances."""
        ...
        # Reset via testmode function (handles async reset)
        _reset_direct_circuit_breaker_instances()
    ...
    @pytest.mark.asyncio
    async def test_async_breaker_full_reset_path(self) -> None:
        """Test AsyncCircuitBreaker reset via full reset_circuit_breaker_state."""
        breaker = AsyncCircuitBreaker(failure_threshold=1)
...
class TestMixedCircuitBreakerReset(FoundationTestCase):
    ...
    @pytest.mark.asyncio
    async def test_sync_and_async_breakers_reset_together(self) -> None:
        """Test that both sync and async breakers reset correctly together."""
        ...
        # Reset both via testmode function
        _reset_direct_circuit_breaker_instances()
...
class TestCircuitBreakerResetNoWarnings(FoundationTestCase):
    ...
    @pytest.mark.asyncio
    async def test_async_breaker_reset_no_warning(self) -> None:
        """Test that resetting AsyncCircuitBreaker doesn't warn about unawaited coroutines.

        This test ensures that _reset_direct_circuit_breaker_instances properly
        awaits AsyncCircuitBreaker.reset() instead of leaving it as an unawaited coroutine.
        """
        breaker = AsyncCircuitBreaker(failure_threshold=1)
        ...
        # Reset - should not produce warnings
        # This call should properly await the async reset without warning
        _reset_direct_circuit_breaker_instances()
